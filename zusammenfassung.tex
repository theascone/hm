			...
			\subsubsection{Lineare Differentialgleichungen n-ter Ordnung mit konstanten Koeffizienten}
				...
				\begin{enumerate}[1.]
					...
					\item Komplexe Beträge zum Fundamentalsystem
						...
						\begin{math}
							e^{\alpha_jx} \cos (\beta_jx), x e^{\alpha_jx} \cos (\beta_jx), \dots, x^{k_j-1} e^{\alpha_jx} \cos (\beta_jx)
						\end{math}
						
						\begin{math}
							e^{\alpha_jx} \sin (\beta_jx), x e^{\alpha_jx} \sin (\beta_jx), \dots, x^{k_j-1} e^{\alpha_jx} \sin (\beta_jx)
						\end{math}
